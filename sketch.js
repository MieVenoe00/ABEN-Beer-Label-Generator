...
function setup() {
  ...
  createP("Noise skala — lav: blød form / høj: kaotisk form")
    .class("sliderLabel")
    .parent("p5Input");
  ...
  createP("Zig-zag — lav: blød / høj: kantet")
    .class("sliderLabel")
    .parent("p5Input");
  zigzag = createSlider(1, 30, 15, 1);
  zigzag.parent("p5Input");

  createP("Ring afstand — lav: tætte ringe / høj: få ringe")
    .class("sliderLabel")
    .parent("p5Input");
...
function draw() {
  ...
  for (let y = height + peakHeight; y > -peakHeight; y -= ringAfstand.value()) {
    ...
    for (let x = 0; x < width; x += zigzag.value()) {
      ...
      const offset = map(
        noise(
          cos(vinkel) * skala.value() + 100,
          sin(vinkel) * skala.value() + 100,
          y * 0.0025,
        ),
        0,
        1,
      ...
      vertex(
        width / 2 + cos(vinkel) * (220 + offset),
        y + sin(vinkel) * (100 + offset),
      );
    }
    endShape(CLOSE);
  }
