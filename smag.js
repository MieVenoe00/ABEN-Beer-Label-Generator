...
function opdaterP5Palette() {
  ...
  window.aktivPalette = farver.length >= 2 ? farver
    : farver.length === 1 ? ["#FFFFFF", farver[0]]
    : null;
}
...
function renderSmagsPicker() {
  ...
  for (const kat of smagsKategorier) {
    ...
    katBtn.onclick = () => { åbenKategori = erÅben ? null : kat.navn; åbenUnder = null; renderSmagsPicker(); };
    ...
    if (kat.underkategorier) {
      for (const under of kat.underkategorier) {
        ...
        underBtn.onclick = () => { åbenUnder = erUnderÅben ? null : under.navn; renderSmagsPicker(); };
...
function renderChips(smage) {
  ...
  for (const s of smage) {
    ...
    chip.className = "smagsChip" + (valgt ? " valgt" : "") + (fuld ? " disabled" : "");
    chip.disabled  = fuld;
